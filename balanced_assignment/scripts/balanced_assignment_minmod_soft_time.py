...
class Servicio:
    ...
    def load(self, row):
        ...
        self.avg_velocity = self.kilometros/(self.horario_llegada-self.horario_salida)

class BalancedAssignmentInstance:
    def __init__(self,U,M,V,w):
...
def get_instance_data(U,M,V,w):
    file_location = "input_balanced_assignment_mini.txt"
    instance = BalancedAssignmentInstance(U,M,V,w)
...
def add_constraint_matrix(my_problem, data):
    ...
    #Calculamos la asignación "justa" por excelencia (el promedio)
    D = 0
    ...
    #Por cada chofer, agregamos dos restricciones de las variables y de la fc objetivo
    # que son las que fuerzan a que se minimice el módulo.
    ...
    for j in range(data.U):
        ...
        my_problem.linear_constraints.add(lin_expr = [row], senses = ['G'], rhs = [-avg_km])
    ...
    #Restriccion para definir variables auxiliares                   
                            
                        else:
                            ind = [data.var_idx[(i1,j,k)],data.var_idx[(i2,j,k+1)],data.var_idx[(i1,i2)]]
                            values = [1,1,-1]
...
def populate_by_row(my_problem, data):
    ...
    # Generamos los indices de las variables auxiliares para las restricciones soft
    for i1 in range(data.cantidad_servicios):
        for i2 in range(data.cantidad_servicios):
            if i1 != i2:
                #Calculamos la holgura que tiene el chofer para llegar de un servicio al otro
                ...
                if holgura >= 0:
                    ...
                    data.var_idx[(i1,i2)] = var_cnt

                    obj.append(data.w * time_diff)
    ...
    # Util para debug.
    my_problem.write('balanced_assignment_soft_time_mini.lp')
...
def main():
    ...
    data = get_instance_data(U,M,V,w)
