...
def get_instance_data(U,K,V):
    #file_location = sys.argv[1].strip()
    file_location = "input_balanced_assignment_mini.txt"
    instance = BalancedAssignmentInstance(U,K,V)
    ...
    for j in range(data.U):
        for k in range(data.K-1):
            for i1 in range(data.cantidad_servicios):
                for i2 in range(data.cantidad_servicios):
                    if i2 != i1:
                        ind = [data.var_idx[(i1,j,k)],data.var_idx[(i2,j,k+1)],data.var_idx[(i1,i2,j,k)]]
                        values = [1,1,-1]
...
def populate_by_row(my_problem, data):
    ...
    # Generamos los indices de las variables AUXILIARES y sus coeficientes en la fc objetivo
    for j in range(data.U):
        for k in range(data.K-1):
            for i1 in range(data.cantidad_servicios):
                for i2 in range(data.cantidad_servicios):
